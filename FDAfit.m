function [ functional_data ] = FDAfit(datavector,timevector,splinestruct,estimation_type,lambda,Lfd_object,weight_vector)
...
%set up roughness penalty smoothing function
        if exist('Lfd_object','var') == 0
            Lfd_object = int2Lfd(4); %penalizes the 4th derivative, helps estimate acceleration functions
...
%generate fine predictions for data,velocity,and acceleration
        functional_data.timefine = linspace(rng(1),rng(2),length(splinestruct.knots)*5);
