function [group1_data, group2_data] = SimulateTwoBiGroupDataBySign(group1_subjectsize,group2_subjectsize,effsize,effsizesubgroup1,effsizesubgroup2)
...
for i = size(effsize,1)+1:size(effsize,1)+size(effsizesubgroup1,1)
    ...
    group1_data(floor(group1_subjectsize/2)+1:group1_subjectsize,i) = group1_data(floor(group1_subjectsize/2)+1:group1_subjectsize,i) - effsizesubgroup1(count);
end
count = 0;
for i = size(effsize,1)+size(effsizesubgroup1,1)+1:size(effsize,1)+size(effsizesubgroup1,1)+size(effsizesubgroup2,1)
    ...
    group2_data(floor(group2_subjectsize/2)+1:group2_subjectsize,i) = group2_data(floor(group2_subjectsize/2)+1:group2_subjectsize,i) - effsizesubgroup2(count);
end
end