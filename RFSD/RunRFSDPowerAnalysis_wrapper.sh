...
sim_type=${sim_type:-'estimated'}
input_data_variable=${input_data_variable:-'input_data'}
group_data_variable=${group_data_variable:-'group_data'}
covariance_matrix_path=${covariance_matrix_path:-'NONE'}
covariance_matrix_variable=${covariance_matrix_variable:-'NONE'}
mu_group_path=${mu_group_path:-'NONE'}
mu_group_variable=${mu_group_variable:-'NONE'}
...
${matlab_command} -nodisplay -nosplash -r "addpath('"${repopath}"') ; RunRFSDPowerAnalysis('SimType','"$sim_type"','CovarianceMatrix',struct('path','"${covariance_matrix_path}"','variable','"${covariance_matrix_variable}"'),'MuGroups',struct('path','"${mu_group_path}"','variable','"${mu_group_variable}"'),'InputData',struct('path','"${input_data}"','variable','"${input_data_variable}"'),'"$groupby_flag"',struct('path','"${group_data}"','variable','"${group_data_variable}"'),'Categorical','"$categorical_vector"','NumSimCases',"$num_sim_cases",'DataRange',"$data_range",'OutputDirectory','"$output_directory"','PerformanceThresholds',"$performance_thresholds",'ForestType','"$forest_type"','LearningType','"$learning_type"','NumCores',"$num_cores",'NumSimulations',"$num_sims",'OutcomeColumnForRegression',"$outcome_regression_column",'InfomapFile','"$infomapfile"','CommandFile','"$command_file"','"$zscore"'); exit"
