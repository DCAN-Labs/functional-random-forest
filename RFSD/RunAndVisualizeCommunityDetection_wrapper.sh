...
${matlab_command} -nodisplay -nosplash -singleCompThread -r "addpath('"${repopath}"') ; RunAndVisualizeCommunityDetection(struct('path','"${corrmatpath}"','variable','"${corrmatvar}"'),'"$filename"','"$infomap_command_file"',100,'LowDensity',"$lowdensity",'StepDensity',"$stepdensity",'HighDensity',"$highdensity",'InfomapFile','"$infomapfile"','InfomapNreps',"$infomap_nreps"); exit"
